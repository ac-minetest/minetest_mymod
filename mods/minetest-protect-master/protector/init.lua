				...
				if infolevel == 1 then
					...
					if playerdata[name]~=nil and meta:get_int("penalty")==0 then 
						if playerdata[name].jail + 0.55 >1 and votingpoll.state ~=1 then
							...
							playerdata[name].jail = playerdata[name].jail + 0.55 
						end
...
function minetest.node_dig(pos, node, digger)
	...
	if node.name ~= protector.node then
		ok = protector.can_dig(5,pos,digger) -- rnd here it checks for dig radius?
	else
...
minetest.register_craft({
	...
	recipe = {
		...
		{"default:stone","default:mese_block","default:stone"}, -- rnd change recipe :)
		{"default:stone","default:stone","default:stone"},
	}
})
minetest.register_craft({
