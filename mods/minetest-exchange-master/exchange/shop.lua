exchange.shop = {}
exchange.shop.current_shop = {}
exchange.shop.formspec = {
...
exchange.shop.check_privilege = function(listname,playername,meta)
...
exchange.shop.give_inventory = function(inv,list,playername)
...
exchange.shop.cancel = function(meta)
	--[[exchange.shop.give_inventory(meta:get_inventory(),"pl1",meta:get_string("pl1"))
	exchange.shop.give_inventory(meta:get_inventory(),"pl2",meta:get_string("pl2"))
	meta:set_string("pl1","")
	meta:set_string("pl2","")
...
exchange.shop.exchange = function(meta)
	--[[exchange.shop.give_inventory(meta:get_inventory(),"pl1",meta:get_string("pl2"))
	exchange.shop.give_inventory(meta:get_inventory(),"pl2",meta:get_string("pl1"))
	meta:set_string("pl1","")
	meta:set_string("pl2","")
	meta:set_int("pl1step",0)
	meta:set_int("pl2step",0)]]
...
minetest.register_node("exchange:shop", {
	description = "Exchange shop",
	tiles = {"exchange_shop_top.png", "default_chest_top.png", "exchange_shop_side.png"},
	...
	on_rightclick = function(pos, node, clicker, itemstack)
		...
		exchange.shop.current_shop[clicker:get_player_name()] = pos
		local meta = minetest.env:get_meta(pos)
		if clicker:get_player_name() == meta:get_string("owner") and not clicker:get_player_control().aux1 then
			minetest.show_formspec(clicker:get_player_name(),"exchange:shop_formspec",exchange.shop.formspec.owner(pos))
		else
			minetest.show_formspec(clicker:get_player_name(),"exchange:shop_formspec",exchange.shop.formspec.customer(pos))
		end
	end,
...
minetest.register_on_player_receive_fields(function(sender, formname, fields)
	if formname == "exchange:shop_formspec" then
		local name = sender:get_player_name()
		local pos = exchange.shop.current_shop[name]
		local meta = minetest.env:get_meta(pos)
