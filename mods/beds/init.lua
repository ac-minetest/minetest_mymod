	...
	on_destruct = function(pos)
	 ...
	 on_rightclick = function(pos, node, clicker, itemstack)
		...
		if minetest.env:get_timeofday() > 0.2 and minetest.env:get_timeofday() < 0.805 then
			...
			-- rnd change: sethome by sleeping in bed
			local player = clicker
			local pos = player:getpos()
			homepos[player:get_player_name()] = pos -- PROBLEM HERE:  doesnt know what homepos is!! homepos is define in sethome mod, solution: make sure sethome mod is run first...by putting it in depends.txt
			 minetest.chat_send_player(player:get_player_name(), "home set. use /home to return here.")
		end
