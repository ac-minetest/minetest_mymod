...
minetest.register_entity("shooter:turret_entity", {
	...
	fire = function(self)
		...
		-- if not inv:contains_item("main", "tnt:tnt") then
			...
			-- return
		-- end
		minetest.sound_play("shooter_shotgun", {object=self.object,max_hear_distance = 256,gain = 2.0}) 
		...
		if obj then
			local ent = obj:get_luaentity()
			if ent then
				minetest.sound_play("shooter_rocket_fire", {pos=pos,gain=1.0,max_hear_distance = 256,})
