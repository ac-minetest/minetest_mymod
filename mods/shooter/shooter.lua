...
SHOOTER_ENABLE_BLASTING = false --false
...
SHOOTER_ALLOW_NODES = false -- true
SHOOTER_ALLOW_ENTITIES = true -- normally: false
	...
	minetest.register_tool(name, {
		...
		on_use = function(itemstack, user, pointed_thing)
			if itemstack:get_wear()+wear < max_wear then
				...
				if uname ~= nil then
					local level = get_level(playerdata[uname].xp);
					if level< 5 then
						itemstack:add_wear(65535/4)
						minetest.chat_send_player(uname, " Your inexperience damages the gun. Need at least experience level 5, check level with /xp")
					end
				end
			else
